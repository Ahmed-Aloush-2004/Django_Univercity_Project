...
def daily_sales_batch_processing():
    """
    Generates a daily sales report and emails it to the admin.
    Memory leaks prevented by explicitly closing IO buffers after sending.
    Distributed lock is cleared in the finally block.
    """
    try:
        ...
        # مسح القفل الموزع من Redis للسماح بطلبات جديدة مستقبلاً
...
# ------------------------------------------------------------------ #
@monitor_performance  # <--- تمت الإضافة هنا لمعرفة وقت معالجة الباتش اليومي
...
def _build_sales_csv(start_date, end_date):
    ...
    # Added iterator() here to prevent loading the entire week's orders into RAM
...
def _build_top_products_chart(start_date, end_date):
    ...
    plt.close(fig)  # Closes the plot memory
    buf.seek(0)
