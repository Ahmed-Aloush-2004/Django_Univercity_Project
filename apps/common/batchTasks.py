...
def daily_sales_batch_processing():
    try:
        # قفل تفادي التشغيل المزدوج للمهمة بنفس الوقت
        ...
        with tempfile.NamedTemporaryFile(mode='w+', delete=False, suffix='.csv', encoding='utf-8') as prod_file, \
            ...
            # إرفاق الملفات مباشرة من القرص الصلب دون استهلاك للذاكرة
...
# تحسين: الكتابة تدفقياً لقيم خام وتفادي ثقل كائنات الـ ORM المعقدة
...
# السحر الهندسي هنا: تدمير الحلقات المتداخلة والاستعلام من الـ OrderItem مباشرة بـ JOIN واحد صريح!
def _build_sales_csv(start_date, end_date, file_object):
    writer = csv.writer(file_object)
    writer.writerow(['Order ID', 'Customer', 'Product', 'Qty', 'Status', 'Total', 'Date'])

    # استعلام مسطح سريع جداً
    sales_data = (
    ...
    for row in sales_data:
        ...
        if status == 'completed':
            items_sold += qty
            # حساب الريفينيو مرة واحدة لكل طلب لمنع تكرار الجمع بسبب انضمام الـ Items
